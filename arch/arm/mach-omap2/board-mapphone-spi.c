...
static void omap_enable_usbhs_extpwr(int val)
{
	if (mapphone_usbhs_ext_pwr.supported) {
		if (mapphone_usbhs_ext_pwr.ext_pwr_enabled != val) {
			gpio_set_value(
				mapphone_usbhs_ext_pwr.gpio_usbhs_ext_pwr, val);
			mapphone_usbhs_ext_pwr.ext_pwr_enabled = val;
		}
	}
}

void usbhs_external_power_enable(int val)
{
	omap_enable_usbhs_extpwr(val);
}
