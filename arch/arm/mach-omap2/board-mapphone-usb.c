...
static struct qsc6085_usb_platform_data qsc_usb_pdata = {
	.modem_interface = 4,
	.remote_wake_gpio = -1
};
...
void __init mapphone_usbhost_init(char *boot_mode)
{
	struct device_node *node;
	const void *prop;
	...
	if (rwkup_gpio >= 0) {
		...
		if (!strncmp(boot_mode, "bp-tools", BOOT_MODE_MAX_LEN))
			mdm6600_usb_pdata.disable_autosuspend = 1;
