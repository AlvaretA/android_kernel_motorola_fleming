...
static void l3_dump_abe_context(void)
{
	/*additional debug info for L3:ABE standard error*/
	pr_err("PM_ABE_PWRSTCTRL : 0x%08x\n",
		readl(OMAP4430_PM_ABE_PWRSTCTRL));
	pr_err("PM_ABE_PWRSTST   : 0x%08x\n", readl(OMAP4430_PM_ABE_PWRSTST));
	pr_err("CM1_ABE_CLKSTCTRL : 0x%08x\n",
		readl(OMAP4430_CM1_ABE_CLKSTCTRL));
	pr_err("CM1_ABE_AESS_CLKCTRL : 0x%08x\n",
		readl(OMAP4430_CM1_ABE_AESS_CLKCTRL));
	pr_err("CM_CLKSEL_ABE : 0x%08x\n", readl(OMAP4430_CM_CLKSEL_ABE));

	show_state();/*long time .. but useful info*/
	/*end debug info*/
}
