...
static int qtouch_force_interrupt_line_high(struct qtouch_ts_data *ts)
{
	struct qtm_object *obj;
	int ret;

	/* configure the SPT_COMMSCONFIG object. */
	obj = find_obj(ts, QTM_OBJ_SPT_COM_CONFIG);
	if (obj && obj->entry.num_inst > 0) {
		struct spt_comms_config_cfg cfg;
		cfg.ctrl = 0x00;
		cfg.command = 0x02;
		ret = qtouch_write_addr(ts, obj->entry.addr, &cfg,
					min(sizeof(cfg), obj->entry.size));
		if (ret != 0) {
			pr_err("%s: Can't write communication command\n",
			       __func__);
		}
	} else {
		pr_err("%s: Can't get SPT_COMMSCONFIG object\n", __func__);
		ret = -ENOENT;
	}
	return ret;
}
...
static int qtouch_ts_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	/*
	 * Ensure the interrupt line is HIGH when entering suspend as
	 * a stuck LOW interrupt line causes increased retention current
	 * drain.  It is possible for the interrupt line to get stuck LOW
	 * if user is touching the screen when the touch panel is
	 * turned off.
	 */
	if (ts->family_id == QTM_OBP_TABLET_FAMILY_ID) {
		ret = qtouch_force_interrupt_line_high(ts);
		if (ret != 0)
			pr_err("%s: Force High pin Failed:%d\n", __func__, ret);
	}
