...
static int _cyttsp4_xy_worker(struct cyttsp4 *ts)
{
	...
	if (num_cur_tch) {
		...
		/*
		 * if no touches were reported, don't send extra sync
		 * --wait for num_cur_touches = 0
		 */
		if (reported_touches)
			input_sync(ts->input);
	} else {
	...
	case CY_SLEEP_STATE:
		...
		if (ts->cmd_rdy) {
			...
			retval = cyttsp4_read_block_data(ts, CY_REG_BASE,
				...
				false);
			if (retval < 0) {
				pr_err("%s: Read failure again (r=%d)\n",
				__func__, retval);
				retval = -EIO;
			} else {
				pr_err("%s: timeout waiting for cmd rdy"
				" interrupt\n", __func__);
			}
		}
