		...
		const uint8_t *buf, int size)
{
	...
	for (i = 1; i <= ATMXT_I2C_ATTEMPTS; i++) {
		...
		if (err < 0) {
			...
			/*
			 * IKHSS6UPGR-12600: Hack to reboot the phone when
			 * we start getting contineous i2c timeout errors.
			 */

			if (err == -ETIMEDOUT)
				if (++i2c_timeout_count >
						ATMXT_MAX_I2C_TIMEOUT_COUNT)
					panic("Unrecoverable I2C HW timeouts");
