...
static uint8_t lp8550_check_for_fault(struct lp8550_data *led_data)
{
	uint8_t ret = 0;
	if (gpio_get_value(led_data->led_pdata->fault_gpio) == 0) {
		/* reading the fault register clears the fault status */
		lp8550_read_reg(led_data, LP8550_FAULT, &ret);
		pr_warn("%s: fault detected [0x%x] & cleared\n", __func__, ret);
	}
...
static void lp8550_early_suspend(struct early_suspend *h)
{
	...
	lp8550_check_for_fault(led_data);
...
{
	struct lp8550_data *n = container_of(p,
					struct lp8550_data, notifier);
	struct led_classdev *led = &n->led_dev;
	...
	switch (event) {
	case FB_EVENT_BLANK:
		new_status = *blank ? BL_BLANK : BL_UNBLANK;
		if (new_status == n->blank_status)
			break;

		if (new_status == BL_BLANK) {
			n->store_brightness = n->brightness;
			led_set_brightness(led, LED_OFF);
		} else
			led_set_brightness(led, n->store_brightness);

		n->blank_status = new_status;
...
#ifdef CONFIG_FB
	led_data->brightness = pdata->power_up_brightness;
	led_data->blank_status = BL_UNBLANK;
	memset(&led_data->notifier, 0, sizeof(led_data->notifier));
	led_data->notifier.notifier_call = fb_notifier_callback;
	error = fb_register_client(&led_data->notifier);
	if (error)
		pr_err("%s: unable to register fb callback: %d\n",
		       __func__, error);
#endif
