 ...
 *      CHARGE_VLCL_RESP     : Voltage level/ Current limit response
 *      CHARGE_VLCL_ACK_RESP : Voltage level/ Current limit request succeded
 *      CHARGE_VLCL_NAK_RESP : Voltage level/ Current limit request failed
 *
				...
				struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE,
		"%s\n",
		data->charger_data.charger_msg);
}
static DEVICE_ATTR(charger_msg, S_IRUGO, charger_msg_show, NULL);


static ssize_t charger_capability_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct switch_dev *csdev = dev_get_drvdata(dev);
	struct cpcap_usb_det_data *data =
	...
	return snprintf(buf, PAGE_SIZE, "%s\n",
			data->charger_data.charger_capability);

}

static DEVICE_ATTR(charger_capability, S_IRUGO, charger_capability_show, NULL);
...
static void whisper_audio_check(struct cpcap_usb_det_data *data)
{
	...
	if ((!switch_get_state(&data->dsdev)) ||
	    (switch_get_state(&data->dsdev) == WHISPER_CHARGER))
		return;
...
static void usb_adapter_vbus_switch(struct cpcap_usb_det_data *data,
								int vbusSw)
{
	int retval = -EFAULT;
	...
	retval |= cpcap_regacc_write(data->cpcap, CPCAP_REG_VUSBC,
				vbusSw ? CPCAP_BIT_VBUS_SWITCH : 0,
				CPCAP_BIT_VBUS_SWITCH);
	if (!vbusSw) {
		retval |= cpcap_regacc_write(data->cpcap, CPCAP_REG_USBC3,
			0, CPCAP_BIT_VBUSEN_SPI);
	}

	if (retval)
		dev_err(&data->cpcap->spi->dev, "VBUS Switch Failed\n");

	msleep(10);
}

static void usb_adapter_dp_pullup_en(struct cpcap_usb_det_data *data,
								int en)
{
	int retval = -EFAULT;
	...
	retval |= cpcap_regacc_write(data->cpcap, CPCAP_REG_USBC1,
			en ? CPCAP_BIT_DP150KPU : 0, CPCAP_BIT_DP150KPU);

	if (retval)
		dev_err(&data->cpcap->spi->dev, "DP NoPull Failed\n");
}
...
static void detection_work(struct work_struct *work)
{
	...
	case IDENTIFY:
		...
		if (data->sense == SENSE_USB) {
			...
			usb_adapter_dp_pullup_en(data, 0);
			data->state = USB_ADAPTER;
			data->adapter_mode = NONE_MODE;
			queue_delayed_work(data->wq, &data->work,
	...
	case USB_ADAPTER:
		/* If the attached USB HDD draws high current instantaneously,
		 * a DC-DC converter such as MAX8815 drops its output volt
		 * and it hangs the HW that use the voltage.
		 * This S/W workaround limits the ability of supplying the
		 * current(under 100mA) using CPCAP path when the attached
		 * USB device starts up. And then when the attached device pulls
		 * up the DP or DM, the CPCAP path is changed to external path
		 * to supply 5V(at least 500mA).
		 */
		pr_cpcap_usb_det(STATUS, "detection_work: USB Adapter\n");
		get_sense(data);
		if (!(data->sense & SENSE_USB_ADAPTER)) {
			pr_cpcap_usb_det(STATUS,
			"detection_work: Adapter Unplugged: %x\n", data->sense);

			data->state = CONFIG;
			data->adapter_mode = NONE_MODE;
			pdata->usbhs_ext_pwr->usbhs_ext_pwr_en(0);
			usb_adapter_vbus_switch(data, 0);
			queue_delayed_work(data->wq, &data->work, 0);
		} else if ((data->adapter_mode != NONE_MODE) && (data->sense &
				(CPCAP_BIT_DP_S_LS | CPCAP_BIT_DM_S_LS))) {
			pr_cpcap_usb_det(STATUS,
			"detection_work: ACCY Plugged: %x\n", data->sense);

			cpcap_irq_mask(data->cpcap, CPCAP_IRQ_DPI);
			cpcap_irq_mask(data->cpcap, CPCAP_IRQ_DMI);
			if (data->adapter_mode == ADAPTER_INIT_MODE)
				notify_accy(data, CPCAP_ACCY_USB_DEVICE);
			else if (data->adapter_mode == ADAPTER_ONLY_MODE) {
				pdata->usb_mux->configure_switch_muxmode(
								OTG_DM_DP);
				pdata->usb_mux->configure_otg_muxmode(USB_OTG);
			}
			pdata->usbhs_ext_pwr->usbhs_ext_pwr_en(1);
			usb_adapter_vbus_switch(data, 0);
			data->adapter_mode = ACCY_MODE;
			cpcap_irq_unmask(data->cpcap, CPCAP_IRQ_IDGND);
			cpcap_irq_unmask(data->cpcap, CPCAP_IRQ_VBUSVLD);
		} else {
			pr_cpcap_usb_det(STATUS,
			"detection_work: Adapter Plugged: %x\n", data->sense);

			if (data->adapter_mode == ACCY_MODE) {
				cpcap_irq_mask(data->cpcap, CPCAP_IRQ_VBUSVLD);
				pdata->usb_mux->configure_switch_muxmode(
								CPCAP_DM_DP);
				pdata->usb_mux->configure_otg_muxmode(
								SAFE_MODE);
				usb_adapter_dp_pullup_en(data, 0);
				pdata->usbhs_ext_pwr->usbhs_ext_pwr_en(0);
				data->adapter_mode = ADAPTER_ONLY_MODE;
			} else
				data->adapter_mode = ADAPTER_INIT_MODE;

			usb_adapter_vbus_switch(data, 1);
			cpcap_irq_unmask(data->cpcap, CPCAP_IRQ_IDGND);
			cpcap_irq_unmask(data->cpcap, CPCAP_IRQ_DPI);
			cpcap_irq_unmask(data->cpcap, CPCAP_IRQ_DMI);
		}
	...
	if ((data->state == CHARGER) ||
		...
		if (!(req->cmd & CPCAP_WHISPER_ENABLE_UART) ==
			...
			if (dock != WHISPER_CHARGER) {
				pdata->usb_mux->
				  configure_switch_muxmode(CPCAP_DM_DP);
				pdata->usb_mux->
				  configure_otg_muxmode(SAFE_MODE);
			}
		       ...
		       struct cpcap_charger_request *req)
{
	...
	pr_cpcap_usb_det(STATUS, "%s: ioctl cmd = 0x%04x\n",
			   __func__, req->cmd);
	switch (req->cmd) {
	case CMD_CHARGER_WHISPER:
		pr_cpcap_usb_det(STATUS,
				 "%s:Sending charge capability uevent\n",
				 __func__);
	...
	case CMD_CHARGER_CAPABILITY:
		strcpy(data->charger_data.charger_capability,
		       req->charger_capability);
		break;

	case CMD_CHARGER_VLCL_REQ:
		strcpy(data->charger_data.charger_msg, req->charger_msg);
		if (!kstrtouint(req->charger_msg, 16, &info)) {
			pr_cpcap_usb_det(STATUS, "Try to switch to %dmV, %dmA",
				(unsigned int)(info/(1<<16)),
				(unsigned int)(info%(1<<16)));
			switch_set_state(&data->csdev,  CHARGE_VLCL_REQ);
		} else {
			pr_cpcap_usb_det(STATUS, "Invalid message, ignore.");
			retval = -EFAULT;
		}
		break;

	case CMD_CHARGER_VLCL_RESP:
		strncpy(reqVLCL, data->charger_data.charger_msg,
			 sizeof(reqVLCL));
		reqVLCL[8] = 0;
		strcpy(data->charger_data.charger_msg,
		       req->charger_msg);
		if (strncmp(req->charger_msg, reqVLCL, sizeof(reqVLCL)) == 0) {
			switch_set_state(&data->csdev, CHARGE_VLCL_ACK_RESP);
			pr_cpcap_usb_det(STATUS,
				"Switch voltage/current succeeded");
		} else {
			switch_set_state(&data->csdev, CHARGE_VLCL_NAK_RESP);
			pr_cpcap_usb_det(STATUS,
					 "Switch voltage/current failed");
		}
		break;
...
static int __init cpcap_usb_det_probe(struct platform_device *pdev)
{
	...
	retval = device_create_file(data->csdev.dev,
				    &dev_attr_charger_capability);
	if (retval < 0) {
		dev_err(&pdev->dev,
			"Failed to create charger_capability file\n");
		goto free_charger_data;
	}
	retval = device_create_file(data->csdev.dev, &dev_attr_charger_msg);
	if (retval < 0) {
		dev_err(&pdev->dev, "Failed to create charger_msg file\n");
		goto free_charger_capability;
	}
...
free_charger_msg:
	device_remove_file(data->csdev.dev, &dev_attr_charger_msg);
free_charger_capability:
	device_remove_file(data->csdev.dev, &dev_attr_charger_capability);
free_charger_data:
