...
static struct notifier_block i2c_rproc_nb;
static struct omap_i2c_dev *i2c_devices[OMAP_I2C_MAX_CONTROLLERS];
...
omap_i2c_probe(struct platform_device *pdev)
{
	...
	if (i2c_dev_count < OMAP_I2C_MAX_CONTROLLERS)
		i2c_devices[i2c_dev_count++] = dev;
	dev->hw_spinlock = 0;
...
static int omap_i2c_rproc_events(struct notifier_block *this,
				unsigned long type, void *data)
{
	int i, r;
	struct omap_i2c_dev *dev;
	if (type == RPROC_ERROR) {
		pr_err("%s: Ducati crashed!notify in I2C driver\n", __func__);
		/*
		 * Check each I2C device if it's holding a hw spinlock,
		 * if not release it
		 */
		for (i = 0; i < OMAP_I2C_MAX_CONTROLLERS; i++) {
			dev = i2c_devices[i];
			spin_lock(&i2c_lock);
			if (dev && !dev->hw_spinlock) {
				spin_unlock(&i2c_lock);
				/*
				 * At this point we know A9 has not acquired
				 * lock and there is no way to know if Ducati
				 * has,so intsead of just unlocking, lock and
				 * then unlock, irrespective of whether lock
				 * fails
				 */
				r = omap_i2c_hwspinlock_lock(dev);
				if (r != 0) {
					dev_info(dev->dev, "Released spinlock "
					"acquired by Ducati for I2C %d\n", i+1);
				}
				omap_i2c_hwspinlock_unlock(dev);
			} else
			spin_unlock(&i2c_lock);
...
static int __init omap_i2c_rpmsg_init(void)
{
	rproc = rproc_get("ipu");
	if (rproc) {
		i2c_rproc_nb.notifier_call = omap_i2c_rproc_events;
		rproc_event_register(rproc, &i2c_rproc_nb);
	} else
		pr_err("%s: Failed to get remoteproc handle!", __func__);

	return 0;
}

subsys_initcall(omap_i2c_init_driver);
late_initcall(omap_i2c_rpmsg_init);

static void __exit omap_i2c_exit_driver(void)
{
	if (rproc) {
		rproc_event_unregister(rproc, &i2c_rproc_nb);
		rproc_put(rproc);
	}
