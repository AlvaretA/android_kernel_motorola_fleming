...
#define HDMI_EDID_LENGTH_IDX			0x7e
...
#ifdef CONFIG_PANEL_MAPPHONE_OMAP4_HDTV
static ssize_t display_edid_show_bin(struct file *filp, struct kobject *kobj,
				  ...
				  char *buf, loff_t offset, size_t count)
{
	...
	r = dssdev->driver->get_edid(dssdev, buf, count);
	if (r)
		return r;

	return min(buf[HDMI_EDID_LENGTH_IDX]*HDMI_EDID_BLOCK_SIZE +
						HDMI_EDID_BLOCK_SIZE,
				HDMI_EDID_MAX_LENGTH);
}

static struct bin_attribute display_sysfs_bin_attr = {
	.attr = { .name = "edid", .mode = S_IRUGO },
	.size = HDMI_EDID_MAX_LENGTH,
	.read = display_edid_show_bin,
	.write = NULL,
};
#endif
...
#ifdef CONFIG_PANEL_MAPPHONE_OMAP4_HDTV
	r = device_create_bin_file((struct device *)&dssdev->dev,
		&display_sysfs_bin_attr);
#endif
...
#ifdef CONFIG_PANEL_MAPPHONE_OMAP4_HDTV
	device_remove_bin_file((struct device *)&dssdev->dev,
		&display_sysfs_bin_attr);
#endif
