...
static const struct snd_kcontrol_new cpcap_snd_controls[] = {
	...
	SOC_SINGLE_BOOL_EXT("EMU AntiPop",
       ...
       /* Enable sidetone control */
	SOC_SINGLE_BOOL_EXT("Sidetone Control",
		CPCAP_REG_GPIO0, snd_soc_get_cpcap_gpio,
		snd_soc_put_cpcap_gpio),
};
				  ...
				  struct snd_ctl_elem_value *ucontrol)
{
	...
	else  if (reg == CPCAP_REG_GPIO0)
		mask = CPCAP_BIT_GPIO0DRV;
	else {
				  ...
				  struct snd_ctl_elem_value *ucontrol)
{
	...
	gpio_reg = (unsigned int)kcontrol->private_value;
	if (gpio_reg == CPCAP_REG_GPIO4) {
		...
		value = cpcap_audio_reg_read(codec, reg) &
			(CPCAP_BIT_MIC2_CDC_EN | CPCAP_BIT_MIC1_CDC_EN);
		...
		} else if (value == (CPCAP_BIT_MIC2_CDC_EN |
					CPCAP_BIT_MIC1_CDC_EN)) {
			/* Stereo capture. I2S for voice call so we ignore
			 * these bits
			 */
			value = tmp | CPCAP_BIT_MIC1_RX_TIMESLOT0;
		} else {
			/* By default, MIC1 is on slot 0 and MIC2 is
			 * on slot 1
			 */
		...
		/* Reset clock tree */
		err = cpcap_audio_reg_write(codec,
	...
	if (reg == CPCAP_REG_GPIO4)
		...
		if (ucontrol->value.integer.value[0] != 0) {
			...
			value = cache[reg] & ~(CPCAP_BIT_MONO_EXT0 |
					       CPCAP_BIT_MONO_EXT1);
		}
			err = cpcap_audio_reg_write(codec, reg, value);
		if (err)
		...
		struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	if (state->codec_strm_cnt == 1) {
		...
		if (pdata->voice_type == VOICE_TYPE_STE) {
			...
			/* Every other modem gets configuration of */
			/* MDM6600 */
